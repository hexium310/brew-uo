...
pub trait BrewUpdateFormatter {
    fn format(&self) -> Result<String, Error>;
    fn colorize_kind(kind: &str) -> Result<String, Error>;
    fn tabulate(&self, formulae: &[&str]) -> String;
}

#[derive(Clone, Debug)]
pub struct BrewUpdate<'a, T>
where
    T: Terminal,
{
    outdated_data: BrewOutdatedData,
    update_data: BrewUpdateData<'a>,
    terminal: T,
}

impl<'a, T> BrewUpdate<'a, T>
where
    T: Terminal,
{
    pub fn new(update_data: &BrewUpdateData<'a>, outdated_data: &BrewOutdatedData, terminal: T) -> Self {
        BrewUpdate {
...
impl<'a, T> BrewUpdateFormatter for BrewUpdate<'a, T>
where
    T: Terminal,
{
    fn format(&self) -> Result<String, Error> {
